...
It's a complicated little beast with lots of sharp edges and rough patches, but it runs well on Python and fairly well on micropython with **looong** load times building the major internal cross reference structures.  [ Note: preliminary testing seems to indicate that the bulk of load latency is coming from building imported classes and garbage collection rather than building internal structures.  We'll see. ]     

IOEngine demonstates what may be an useful application of binary indexing to build small logic engines.  BIRM Technology ( Binary Indexing Run Mad ) !

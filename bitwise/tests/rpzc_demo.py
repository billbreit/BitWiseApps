...
def display_dbdef(dbdef):
    ...
    print()
    for r in dbdef.relations: print(r)
    print()
    for t in dbdef.table_defs:
        print(t)
        print()
...
if __name__=='__main__':
    ...
    # for r in res.keys():
    #    print(f"Expenditure {r[0]}: {projr.expenditures(r[0])}") 
    ...
    if mem_free_present:
        ...
        '''
        # better than above for memory, but doesn\'t actually delete from locals (???)
        ll = [ dbdef, memdata, projdata, roledata, projmemdata, rdata, prdata, evdata ]
        ...
        '''  # no effect on mem_free
        ll = [ mems, projs, roles, projmems, res, projr, evt, ctab ]
        for _ in range(len(ll)):
            x = ll.pop(0)
            del x
        '''

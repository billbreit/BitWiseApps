...
RelationDef = namedtuple('RelationDef',
                         ['name', 'inverse', symmetric', 'transitive'])
...
class EvaluationError(Exception):
    ...
    def __str__(self):
        return 'EvaluationError( msg = {0}, exception = {1} )'.format(
                         self.valerr_dict['message'],
                         self.valerr_dict['exception'])
...
class Evaluation(object):
    ...
    def typeof(value, *args):
        ...
        return isinstance(value, args[0])

    def not_typeof(value, *args):
        """arg[0] can be a Python type or tuple of types."""

        return not isinstance(value, args[0])
...
class Evaluator(object):
    """Slightly tricky. Subclass needs to do super().__init__() if adding
       app-level evaluations in sublass __init__, via
       self.evaluations.update(_my_val_dict). Don't use names _e and
       _evaluations        as class variables to avoid shared class name
       space problems."""
    ...
    def validate(self, key, value, *args):
        if key in self.evaluations:
            ...
            t_msg = 'Bad Evaluation Name: The evaluation name "{0}" is invalid. ' \
                           .format(key)

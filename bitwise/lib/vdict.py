...
Volatile Dictionary - tracks keys of changed items for notification/synching
    -> between peer [writer -> reader] processes,
    -> possibly two-way processes [peer <-> peer].

    - generally add/modify only, usually all keys will created before using
      dict, such as [one writer -> many readers].
    ...
    -  vdict.popitem not supported.  Key deletes suppported, but not
       recommended for 'volatile' scenarios.  Use DELETED tuple.
    ...
    - Note: micropython base class inheritence is much improved, but it won't
      run on old versions of CircuitPython, try OrderedDict.  However,
    ...
    Some of the design limitations are artificial.  New writer->reader
    relations can be added and deleted at will, but it's not clear why do it
    under most 'tightly coupled' usage scenarios.

    - Some sort of namespacing and 'namedspace merge' might be useful.
...
RO = namedtuple('RO', ['key', 'value'])  # flag k/v tuple as read only
...
class VolatileDict(dict):
    """ Volatile Dictionary, tracks keys of changed items for
        ...
        - Using delete flags rather than del, pop guarantees validity of all
          shared access masks across clients.  Not thread-safe without lock.

        - Can be used like OrderedDict. None in some low-memory upython builds.

        - Can only create vdict from k-v tuple pairs ( not dict ) to maintain
          order of entry, possibly from a database load.  Update does not
          enforce this for possibly unordered dict.
        ...
        - In newer versions of MicroPython, vdict is about as fast as
          collections.OrderedDict.  Faster in older versions.

        - Note: MicroPython base class inheritence is much improved, but vdict
          won't run on CircuitPython which adheres to older versions of mpy.
    """

    def __init__(self, *args, **kwargs):

        super().__init__(*args, **kwargs)
        self.vkeys: list = []  # ensure insert order when OrderedDict not avail
        self.changed: int = 0
        ...
        if len(args) > 0:
            # must use k/v tuples to create, can bypass with vdict.update(dict)
            if isinstance(args[0], dict):
                raise VolatileDictException(
                    'Init Error: can only create vdict from tuple pairs.')
    ...
    def __setitem__(self, key: str, value: 'Any'):
        ...
        if key in self.vkeys and key in self.read_only:
            raise VolatileDictException(
                f"Read-Only Key: Can not change read-only key '{key}'")
    ...
    def __delitem__(self, key: str):
        ...
        if key in self.read_only:
            raise VolatileDictException(
                f"Read-Only Key: Can not delete read-only key '{key}'")
    ...
    def copy(self) -> 'VolatileDict':
        ...
        with self.lock:    # needed in sync world ?
    ...
    def pop(self, key:str) -> 'Any':
        ...
        if key in self.read_only:
            raise VolatileDictException(
                f"Read-Only Key: Can not pop read-only key '{key}'")
        if key not in self:
            raise VolatileDictException(
                f"Key Pop Error: key '{key}' not found.")
    ...
    def update(self, dictorlist):
        ...
        if isinstance(dictorlist, dict) and len(dictorlist) > 0:
            ...
            raise VolatileDictException(
                f"Update Error: update failed for input '{dictorlist}'.")

    def ischanged(self) -> bool:
        return self.changed != 0

    def reset(self, key: str = None):
        """ If no key, set changed to 0.
            If key, AND of NOT slot index into changed """

        if key is not None and key not in self.vkeys:
            raise VolatileDictException(
                f"Key Reset Error: key '{key}' not found.")
    ...
    def fetch(self, keylist:list=None ) -> list['Any']:

...
tdef = TableDef(tname = 'People',
                filename = 'people',
                unique=['name'],
                col_defs = [
                    ColDef(cname='name', default=None, ptype=str),
...
and indexing.
 ...
 - a cross-platform version of namedlist.py, when Python dataclass is not
   available.
...
class TableStore(TupleStore):
    ...
    def __init__(self, tdef: TableDef = None, db: 'DataStore' = None):
        """ Based on TupleStore with:
            ...
            - python types, including namedtuple in columns, recover from JSON
            - JSON data store and recovery of non-JSON types, i.e. tuples

            - when used in DataStore (db not None), enforce referential
              integrity,  parent/child key rels between tables.
        ...
        # if TableStore used in DataStoreDef, then parent/child key relations.
    ...
    def _postinit(self):
        ...
        if self.db:
            ...
            self._prelations = [PRelation(r.pcol,
                                          self.db.tabledict[r.child],
                                          r.ccol)
                                for r in self.db.relations
                                if r.parent == self.tablename]

            # child to parent, answers table row parents_exist ?
            self._crelations = [CRelation(r.ccol,
                                          self.db.tabledict[r.parent],
                                          r.pcol)
                                for r in self.db.relations
                                if r.child == self.tablename]
    ...
    def keys(self) -> list[list]:
        """ Return list of key value lists,
            like [['John','Doe'],['Bob','Smith']]  """
    ...
    def is_duplicate(self, key: list) -> bool:
        ...
        if len(key) != len(self.unique_columns):
            raise TableStoreError(
                f'Key Error: length of key {key} must be {len(self.unique_columns)}.')
    ...
    def validate_row(self, row: list, add: bool = True) -> list:
        ...
        if add:
            if self.is_duplicate(key):
                errors.append(
                    f"Validation Add Error: key '{key}' is duplicate.")
        else:
            if not self.is_duplicate(key):
                errors.append(
                    f"Validation Update Error: key '{key}' does not exist.")
    ...
    def test_type(ptype, value):
        ...
        try:
            ...
            raise TableStoreError(
                f"Bad Python Type: '{value}' is not {ptype}. {e} ")
    ...
    def check_column_types(self, col_name: str, col_values: list = None) -> list:
        ...
        for val in col_values:
            try:
                ...
                err_list.append(
                    f"Validation Error: value '{val}' must be type {ptype}")
    ...
    def validate_types(self, list_in: [list, tuple]) -> list:
        ...
        if not isinstance(list_in, (list, tuple)) or \
           list_in is None or list_in in [[], ()]:
            raise TableStoreError(
                'Invalid List: must provide list, not None or empty')
        ...
        for pt, v in zip(self.ptypes, list_in):
            try:
                ...
                err_list.append(
                    f"Validation Error: value '{v}' must be type {pt}")

        return err_list

    def fix_types(self, data: list, resolve_namedtuples: bool = False) -> list:
        """Restore Python types from JSON.  Resolve namedtuples,
           that is tuple -> namedtuple."""
        ...
        if self.db:
            ...
            for ccol, par, pcol in self._crelations:
                ...
                if par.find(pcol, pkey) < 0:  # may be unique or not, need compund key ?
                    errors.append(
                        f"Invalid Parent Key: key '{pkey}' has no parent key in {par.tablename} table")
        ...
        if self.db:
            ...
            for pcol, child, ccol in self._prelations:
                ...
                if num_ch > 0:   # has children
                    info.append(
                        f"Child Key Dependency: key '{ckey}' has {num_ch} children in {child.tablename} table.")

        return info

    def children_exist(self, row: list) -> bool:
        """Row has dependent rows in child tables, is error if parent
        being row popped/removed.  """
    ...
    def fetch_key(self, key: list) -> tuple:
        """Find unique_row and pass slot to fetch row, resetting changed bit
           for row"""
    ...
    def get_key(self, key: list, asdict: bool = False) -> tuple:
        ...
        if slot < 0:
            raise TableStoreError(
                f"Get Key Error: key '{key}' not found in {self.tablename}.")
    ...
    def set(self, key: list, col_name: str, value):
        ...
        if col_name in self.unique_columns:  # equiv to rename(delete/add ) row
            key_changed = True
            ch_list = self.validate_children(rrow)

            if len(ch_list) > 0:
                raise TableStoreError(
                    f"Set Error; column {col_name} '{key}' has dependent children. {ch_list}.")
        else:
            key_changed = False
        ...
        if len(err_list) > 0:
            raise TableStoreError(
                f"Set Error: Invalid value {value} for {col_name}: ", err_list)
    ...
    def extend(self, list_of_lists: list = None):
        ...
        if list_of_lists is None or len(list_of_lists) == 0:
            raise TableStoreError(
                'Extend - list passed can not be None or empty.')
        ...
        err_list = [e for e in map(self.validate_row, list_of_lists)
                    if e]  # if e != []

        if any(err_list):
            raise TableStoreError(
                'Extend - invalid rows, no update: ', err_list)

        super().extend(list_of_lists)

    def pop(self, key: list) -> tuple:
        """Remove row from TableStore and return row.  Do not allow pop if
           parent row has existing children in related tables. """
        ...
        if len(ch_list) > 0:
            raise TableStoreError(
                f"Pop Error: key '{key}' has dependent children. {ch_list}.")
    ...
    def find_unique(self, unique_key: list) -> int:
        """Return a slot for a row matching list of values in unique key
           columns. Uses first key slot for store.index search """
        ...
        if len(unique_key) != len(self.unique_columns):
            raise TableStoreError(
                f"Find Unique: misformed key '{unique_key}', must have {len(self.unique_columns)} columns." )
...
""" DBDef, DataStore Definition -
      ...
      table_defs:list[TableDef], TableDef subclasses,
                 in parent->child db load order
"""

# mpy has no _fields method
...
""" Parent  Relation, is parent of child, embedded in table -
    no delete if existing children.
      pcol:str,
      child:TableStore,  # looking down from one to many
      ccol:str
"""
# PRel_fields = [ 'pcols', 'child', 'ccols' ]  map list pcols to list col names
PRel_fields = ['pcol', 'child', 'ccol']    # map single name to col,
PRelation = namedtuple('PRelation', PRel_fields)

""" Child Relation, is child of parent, embedded in table -
    no add without existing parent
      ccol:str,
...
class DataStore(object):
    ...
    def __init__(self, dbdef: DataStoreDef = None):
        ...
        if self._dbdef is not None:
            ...
            tdefs = [tclass._constructor()
                     for tclass in self._dbdef.table_defs]
        ...
        if len({tdef.tname for tdef in self.dbdef.table_defs}) != \
           len([tdef.tname for tdef in self.dbdef.table_defs]):
            raise DataStoreError('DataStoreDef contains duplicate tables.')

        # for mpy to maintain order, must load par->child
        self.tabledict = OrderedDict()
        for tdef in self.dbdef.table_defs:
            self.tabledict[tdef.tname] = tdef.ttype(tdef, self)

        # embeds parent->child and child->parent rels into tables
        for tobject in self.tabledict.values():
            tobject._postinit()

        # set attrs for table name/table instance, used for queries.
        # For ex. db.Customer is short hand for db.table('Customer'),
        # of table class Customers
        for n, v in self.tabledict.items():
            if n not in dir(self):
                setattr(self, n, v)
            else:
                raise DataStoreError(
                    f'Table name {n} conflicts with a DataStore attr. name.')
    ...
    def tables_changed(self) -> list[str]:
        return [tn for tn, ti in self.tabledict.items()
                if ti.rows_changed() > 0]

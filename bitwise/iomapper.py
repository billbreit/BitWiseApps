...
""" IOMapperDef
    ...
    This structure is used with values dict as:
        IOMapper(values_dict,*iomapper_def ).
...
iomapper_fields = ['values', 'iomap', 'read_keys', 'local_values',
                   'transforms']

IOMapperDef = namedtuple('IOMapperDef', iomapper_fields)

# Commands for the agenda mechanism, pass actions forward to
# next binding cycle, in order to maintain valid values_changed flags.
...
class IOMapper(object):
    ...
    # for class variables 'subclassing' or specialization,
    # helps keeps namespace uncluttered
    ...
    def __init__(self,
        ...
        if MDEBUG:
            ...
            print(self.values)
            print()
        self.local_vals = self._local_values \
            if self._local_values else local_vals
        ...
        if self.local_vals:
            ...
            #    raise IOMapperError(
            #              'Duplicate keys in values and local_values dicts.')
            # else:
        ...
        self.read_into_values(self.all_vreturns)  # initialize, default 'all'
        if MDEBUG:
    ...
    def run_agenda(self):
        ...
        for command in self.agenda:
            if isinstance(command, SetVal):
                if MDEBUG or EDEBUG:
                    print('     SetVal ', command.key, command.value)
    ...
    def read_into_values(self, read_keys: list = None):
       ...
       """
        ...
        if MDEBUG:
            print('### IOMapper.read_into_values using:', reads)
        ...
        for key in reads:
            ...
            if MDEBUG:
                ...
                print('Read/write action key:', key, ' vreturn:', vreturn,
                      ' chain:', chain)
            v = self.bind(wrap, target, params)  # try/except/handle here ?
            if vreturn and v != self.values[vreturn]:
                if key in self.transforms and v is not None:  # filter None ?
                    value = self.transforms[key](v)
                    if MDEBUG:
                        print(f'calibrating {vreturn} to {value}')
                    self.values[vreturn] = value
                else:
    ...
    def bind(self, wrap, target, params) -> 'Any':
        ...
        if params:
            if isinstance(params, dict):
                ...
                evals = [self.values[p]
                               if isinstance(p, str) and p in self.values
            ...
            if wrap is None:  # is function
                if evals:
                    ...
                    if isinstance(evals, dict):  # by keyword in dict, retest?
                        if MDEBUG:
                            print('In target(dict) - evals ', evals)
                        result = target(**evals)
                    else:
                        result = target(*evals)
                else:
                    if isinstance(target, (deftype, methtype)):  # others ?
                        ...
                        result = target  # property ?, wrapped with attrgetter

            else:  # is setter
                if MDEBUG:
                    print(f'setter for {wrap}, with {target} {evals}')
                result = wrap(target, *evals)

            if MDEBUG:
                print(f'exit bind() - returning -> {result}')
            return result

        raise IOMapperError(
                    f"Can't bind {wrap} | {target} | {params} | {evals} -> .")
...
if __name__ == '__main__':
    ...
    vd = VolatileDict([('room_temp', 10.1),
                       ...
                       RO('led_off_param', {'brightness': led.OFF}),
...
and 'led_state' so values are in sync with fan.ON.")
    ...
    print(
        "Note that phone did not change but the 'phone' in values dict did.")
    nl()

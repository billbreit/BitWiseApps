...
the internal heat generated by the running fan.  The fan motor is really
cheap and burns out at 42C.  The trick is to keep the fan running while
avoidong overheating the fan motor and burning it out.        

A DEAD FAN Exception happens about one in ten runs.  In fact, the exceptions
seems to cluster, happening two or three times in sucession and the not at
all for a dozen or so runs.  Really random ???  Apparently not.  
...
# import iomapper # uncomment for MDEBUG
